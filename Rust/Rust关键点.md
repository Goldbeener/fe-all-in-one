...
## 堆栈存储
栈存储，先进后出
栈中的数据都必须占用已知且固定大小的内存空间

对于大小未知或者可能变化的数据，需要存在堆上
当向堆上放入数据时，需要请求一定大小的内存空间，操作系统在堆的某处找到一块足够大的空位，把它标记为已使用，并返回一个表示该位置地址的指针。该过程称为在堆上分配内存。
接着指针会被推入栈中，后续过程中通过栈中的指针来获取数据在内存中的位置，进而访问数据。


## 所有权

1. rust中每一个值都被一个变量所拥有，该变量称为值的所有者
2. 一个值同时只能被一个变量所拥有，或者一个值只能拥有一个所有者
3. 当所有者离开作用域范围时，值将被丢弃
