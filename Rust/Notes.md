...
`Option<T>` 用于表示**值的存在与否**
`Result<T, E>` 用于关注**值的正确与否**
常用于函数返回值
...
**泛型**
使用泛型，就是为了实现函数的多态；一个函数处理不同的参数

因为rust是静态类型语言，函数声明时必须要指定函数类型
因此正常函数在定义时就确定了类型，之后函数体内可以使用什么操作也由生命的参数类型决定了

使用泛型，相当于是指定了一个函数可能接受的参数类型集合
函数可以根据接受的不同类型的参数，返回不同类型的数据

同时在rust里面，还需要额外注意一点，就是参数必须要指定特征，因为rust内某些操作有限制的，不是任何数据都可以

因此，使用泛型时需要指定，这些**类型都支持的一类特征**，以便rust编译器在编译环节检查错误。

**是数据类型的抽象**，并且抽象的类型也必须有公共特征，以满足通用的操作
...
Trait定义了一组可以被共享的行为，只要实现了特征，就能使用这组行为
...
**特征约束**
搭配范型，约束类型行为，方便在一个方法内处理不同类型但是具有相同行为的的入参对象。


**孤儿规则**
Trait定义与实现的位置
类型和Trait至少有一个是在当前作用域中定义
...
可以通过`&引用`或者`Box<T>`智能指针的方式来创建特征对象


鸭子类型： 并不关注数据精确的类型是什么，如果数据有某些特征，就符合条件
